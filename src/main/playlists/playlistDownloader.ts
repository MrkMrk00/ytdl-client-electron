...
const [readdir, unlink] = [
    promisify(readdirCallback),
    promisify(unlinkCallback),
]
...
export const downloadPlaylist = async (
    ...
    const constructCommandArgs = (indexedEntry: IndexedEntry) => {
        const commandArgs = [
            ...
            path_join(
                Store.get('rootDir'),
                relativePath,
                `${indexedEntry.index + 1} - %(title)s.%(ext)s`
            ),
    ...
    for (const indexedEntry of toDownload) {
        ...
        await ytdl
            .exec(constructCommandArgs(indexedEntry), data =>
                updatePercentageFromOutput(data.toString())
            )
            .catch(e => {
                log(e.message)
    ...
    for (const indexedEntry of threwError) {
        ...
        Store.set(
            ...
            toDownload.length -
                threwError.length +
                threwError.indexOf(indexedEntry) +
                1
        )
    }
...
const filterAlreadyDownloaded = async (relativePath: string) => {
    ...
    const alreadyDownloaded = await readdir(
        path_join(Store.get('rootDir'), relativePath)
    )

    const toDownload: IndexedEntry[] = []
    playlistInfo.entries.forEach((entry: PlaylistEntry, index: number) => {
        const filterResult = alreadyDownloaded.filter(fileName =>
            fileName.includes(entry.title)
        )
        if (filterResult.length === 0)
            toDownload.push({ index: index, entry: entry })
    })
...
const deletePartiallyDownloaded = async (relativePath: string) => {
    ...
    const partiallyDownloaded = dirContents.filter(content =>
        /\.part$/.test(content)
    )

    for (const path of partiallyDownloaded) {
        await unlink(path_join(Store.get('rootDir'), relativePath, path)).catch(
            e => log(e.message)
        )
    }
}
...
const updatePercentageFromOutput = (data: string) => {
    ...
    const percentage = /\[download].*?([0-9]{1,3}\.[0-9])%/gi.exec(data)
